"""
MiniMind v2/v3 结构推理入口（`model_minimind_v2`），用于加载 `out_v2/*.pth`。
遗留权重 `out/*_512.pth` 请用 `eval_llm.py`。
"""
import time
import argparse
import random
...
def main():
    parser = argparse.ArgumentParser(description="MiniMind v2 模型推理与对话（out_v2 权重）")
    parser.add_argument(
        "--load_from",
        default="model",
        type=str,
    ...
    parser.add_argument("--save_dir", default="out_v2", type=str, help="模型权重目录")
    parser.add_argument(
        "--weight",
        default="full_sft_v2",
        type=str,
        help="权重名称前缀（如 pretrain_v2_best, full_sft_v2）",
    )
    ...
    for prompt in prompt_iter:
        ...
        if "pretrain" in args.weight:
            ...
            inputs = tokenizer.apply_chat_template(
                conversation, tokenize=False, add_generation_prompt=True, open_thinking=bool(args.open_thinking)
            )
